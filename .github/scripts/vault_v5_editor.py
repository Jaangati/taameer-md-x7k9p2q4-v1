...
s = css.read_text()
root_old = ':root { --accent: #757575; --accent-soft: rgba(117,117,117,0.12); --accent-hover: rgba(117,117,117,0.22); }'
root_new = ':root { --accent: #757575; --accent-soft: rgba(117,117,117,0.12); --accent-hover: rgba(117,117,117,0.22); --system-dark: #030712; --system-dark-2: #111827; }'
if root_old in s:
    s = s.replace(root_old, root_new, 1)
elif '--system-dark:' not in s:
    raise SystemExit('root token target not found')
css.write_text(s)
...
s = s[:ask_start] + new_ask + s[ask_end:]

media_old = '@media(max-width:760px){.vault-ask-row{flex-wrap:wrap}.vault-ask-copy{min-width:0;width:100%}.vault-ask-input{width:100%}.vault-head{align-items:flex-start}.vault-head-actions{flex-wrap:wrap;justify-content:flex-end}.vault-compose-row{grid-template-columns:1fr}.vault-grid{grid-template-columns:1fr}}'
media_new = '@media(max-width:900px){.vault-compose{width:min(720px,96vw);max-height:92vh}.vault-compose-body{grid-template-columns:1fr}.vault-compose-settings{border-right:0;border-bottom:1px solid #e2e6ec;display:grid;grid-template-columns:1fr 1fr}.vault-compose-main{padding:18px}.vault-compose textarea{min-height:260px}}@media(max-width:760px){.vault-ask-row{flex-wrap:wrap}.vault-ask-copy{min-width:0;width:100%}.vault-ask-input{width:100%}.vault-head{align-items:flex-start}.vault-head-actions{flex-wrap:wrap;justify-content:flex-end}.vault-compose-settings{grid-template-columns:1fr}.vault-grid{grid-template-columns:1fr}}'
if media_old not in s:
    raise SystemExit('responsive CSS target not found')
s = s.replace(media_old, media_new, 1)

editor_start = s.find('  function openEditor(id=null,quick=false){')
editor_end = s.find('  async function saveEditor', editor_start)
if editor_start < 0 or editor_end < 0:
    raise SystemExit('openEditor markers not found')
new_editor = '''  function openEditor(id=null,quick=false){
...
idx = Path('index.html')
h = idx.read_text()
h = re.sub(r'js/vault\.js\?v=[^"\']+', 'js/vault.js?v=20260908-v5', h)
idx.write_text(h)
